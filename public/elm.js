...
var author$project$Main$GetGeoLocation = {$: 'GetGeoLocation'};
...
var author$project$Main$ReverseGeoResult = function (a) {
	return {$: 'ReverseGeoResult', a: a};
};
var elm$json$Json$Decode$field = _Json_decodeField;
var elm$json$Json$Decode$string = _Json_decodeString;
var author$project$Main$reverseGeoLookupDecode = A2(
	elm$json$Json$Decode$field,
	'address',
	A2(elm$json$Json$Decode$field, 'town', elm$json$Json$Decode$string));
...
var author$project$Main$reqReverseGeoLookup = function (location) {
	var lon = elm$core$String$fromFloat(location.longitude);
	var lat = elm$core$String$fromFloat(location.latitude);
	var url = A2(
		elm$core$String$join,
		'',
		_List_fromArray(
			['https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=', lat, '&lon=', lon, '&zoom=18&addressdetails=1&email=havar.novik%40gmail.com']));
	return elm$http$Http$get(
		{
			expect: A2(elm$http$Http$expectJson, author$project$Main$ReverseGeoResult, author$project$Main$reverseGeoLookupDecode),
			url: url
		});
};
...
var billstclair$elm_geolocation$PortFunnel$Geolocation$decode = function (_n0) {
	var tag = _n0.tag;
	var args = _n0.args;
	switch (tag) {
		case 'getlocation':
...
var author$project$Main$send = function (message) {
	return A2(billstclair$elm_geolocation$PortFunnel$Geolocation$send, author$project$Main$cmdPort, message);
};
var billstclair$elm_geolocation$PortFunnel$Geolocation$commander = F2(
	function (_n0, _n1) {
		return elm$core$Platform$Cmd$none;
	});
var billstclair$elm_geolocation$PortFunnel$Geolocation$defaultOptions = {enableHighAccuracy: false, maximumAge: elm$core$Maybe$Nothing, timeout: elm$core$Maybe$Nothing};
var billstclair$elm_geolocation$PortFunnel$Geolocation$nowWith = billstclair$elm_geolocation$PortFunnel$Geolocation$GetLocation;
var billstclair$elm_geolocation$PortFunnel$Geolocation$now = billstclair$elm_geolocation$PortFunnel$Geolocation$nowWith(billstclair$elm_geolocation$PortFunnel$Geolocation$defaultOptions);
...
var author$project$Main$geolocationHandler = F3(
	function (response, state, model) {
		var foo = A2(elm$core$Debug$log, 'model2', response);
		switch (response.$) {
			...
			default:
				return A2(author$project$Main$update, author$project$Main$GetGeoLocation, model);
		}
	});
		...
		switch (msg.$) {
			case 'GetGeoLocation':
				return A2(
					...
					author$project$Main$send(billstclair$elm_geolocation$PortFunnel$Geolocation$now),
					A2(elm$core$Debug$log, 'model', model));
			case 'Process':
				var value = msg.a;
				var _n1 = A5(
					billstclair$elm_port_funnel$PortFunnel$processValue,
					author$project$Main$cyclic$funnels(),
					author$project$Main$appTrampoline,
					value,
					model.state,
					model);
				if (_n1.$ === 'Err') {
					...
					var mdl = _n2.a;
					var cmd = _n2.b;
					var _n3 = mdl.location;
					if (_n3.$ === 'Just') {
						var location = _n3.a;
						return _Utils_Tuple2(
							mdl,
							author$project$Main$reqReverseGeoLookup(location));
					} else {
			...
			default:
				...
				if (httpRes.$ === 'Err') {
					...
					var newCity = httpRes.a;
					return Janiczek$cmd_extra$Cmd$Extra$withNoCmd(
						_Utils_update(
							model,
							{
								city: elm$core$Maybe$Just(newCity)
							}));
				}
		}
	});
function author$project$Main$cyclic$funnels() {
...
try {
	var author$project$Main$funnels = author$project$Main$cyclic$funnels();
	author$project$Main$cyclic$funnels = function () {
		return author$project$Main$funnels;
	};
} catch ($) {
...
var author$project$Main$init = function () {
	var model = {
		...
		state: {geolocation: billstclair$elm_geolocation$PortFunnel$Geolocation$initialState}
	};
	return A2(author$project$Main$update, author$project$Main$GetGeoLocation, model);
}();
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						elm$html$Html$Events$onClick(author$project$Main$GetGeoLocation)
					]),
				_List_fromArray(
					[
